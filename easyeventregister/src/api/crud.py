...
from .models import EventCreate, Event, RegistrationCreate
...
def get_events(db: Session, skip: int = 0, limit: int = 100):
    ...
    return db.query(database.Event).order_by(database.Event.date.desc()).offset(skip).limit(limit).all()
...
def get_registrations_for_event(db: Session, event_id: int):
    ...
    return db.query(database.Registration).filter(database.Registration.event_id == event_id).all()
